...
<template>
  <div :class="`input-add ${props.class}`">
    <textarea
      ...
      id=""
      style="resize: none"
      :style="`${!props.isBig ? 'display: none' : ''}`"
    ></textarea>
    <input type="text" :style="`${props.isBig ? 'display: none' : ''}`" />
    <ActButton class="add" :style="`${props.isButton ? '' : 'display: none'}`"
      ><slot>add smth</slot></ActButton
    >
  </div>
</template>
